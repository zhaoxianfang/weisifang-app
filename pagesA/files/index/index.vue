<template>
    <view class="container">
        <view class="no-data" v-if="current < 1">
            <tui-no-data :fixed="false" imgUrl="/static/images/tabbar/null.png" btnText="查看本地文件"
                @click="openLoaclFiles">
        ...
        <view class="tui-padding" v-if="current > 0">
            <view v-for="(item,index) in filesList" :key="index" class="tui-classify-box">
                <tui-collapse :index="index" :current="collapseCurrent" :disabled="item.disabled"
                    @click="changeCollapse">
                    <template v-slot:title>
                        <tui-list-cell padding="30rpx" :hover="false" unlined>
                            <view class="tui-classify__main">{{item.name}}</view>
                        </tui-list-cell>
                    </template>
                    <template v-slot:content>
                        <view class="tui-classify__sub">
                            <view class="tui-classify__item" @tap="previewFile(file)" v-for="(file,key) in item.list"
                                :key="key">{{file.title}}</view>
                        </view>
                    </template>
                </tui-collapse>
            </view>
