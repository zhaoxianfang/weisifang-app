	...
	<view>
		<tui-bubble-popup :show="popupShow" @close="toggle" :maskBgColor="maskBgColor" right="8px" :top="popupTop" :mask="false" triangleRight="16px" triangleTop="-22rpx">
			<view class="tui-popup-item" :class="{ 'tui-start': index === 0, 'tui-last': index === itemList.length - 1 }"
			 ...
			 :key="index">
				<tui-icon class="tui-bubble-popup_icon" :name="item.icon" color="#fff" :size="40" unit="rpx" v-if="item.icon && !isImage"></tui-icon>
				<image class="tui-bubble-popup_icon" :src="item.icon" v-if="item.icon && isImage" :style="{width:width,height:height}"></image>
